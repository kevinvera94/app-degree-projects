...
async def list_academic_programs(
    ...
    result = await db.execute(
        text(
            ...
            f" FROM public.academic_programs {where} ORDER BY name"
        ),
        params,
    )
...
@router.post(
    "", response_model=AcademicProgramResponse, status_code=status.HTTP_201_CREATED
)
...
async def update_academic_program(
    ...
    result = await db.execute(
        text(
            f"UPDATE public.academic_programs"
